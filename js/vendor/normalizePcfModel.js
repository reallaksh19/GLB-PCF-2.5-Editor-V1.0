...
function normalizeBlock(block, log, idx) {
  const comp = {
    ...
    attributes: { ...block.rawAttrs },  // inherit all raw attributes, we will clean geometry keys
    ep1: null,
    ep2: null,
  ...
  // Clean geometry keys from attributes
  GEOMETRY_KEYS.forEach(k => delete comp.attributes[k]);
  ...
  for (const line of block.lines.slice(1)) {
    ...
    if (trimmed.startsWith('END-POINT')) {
      const match = trimmed.match(/^END-POINT\s*(.*)$/);
      if (match) {
        const val = match[1];
        if (val.includes(' TO ')) {
          const parts = val.split(' TO ');
          const pt1 = parsePoint(parts[0], true);
          const pt2 = parsePoint(parts[1], true);
          if (pt1) endPoints.push(pt1);
          if (pt2) endPoints.push(pt2);
        } else {
          const pt = parsePoint(val, true);
  ...
  if (comp.attributes['BORE'] && Number.isFinite(parseFloat(comp.attributes['BORE']))) {
    comp.bore = parseFloat(comp.attributes['BORE']);
  } else {
...
export function normalizePcfModel(parsed, log) {
  ...
  // Keep track of header attributes to inherit
  const headerAttrs = {};
  if (parsed.blocks.length > 0 && !parsed.blocks[0].type) {
     // Assuming split blocks correctly isolates headers
  }
  ...
  // Post process to inherit pipeline references, temps, etc. from headers if not present
  // But wait, splitPcfBlocks doesn't return header blocks. It returns blocks array where each block is a component.
  // Wait, let's see how splitPcfBlocks parses attributes before the first block.
